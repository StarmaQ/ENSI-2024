...
void calibrate() {
  ...
  while (digitalRead(3) == 1) {
    ...
    // function provided by the qtra library
    qtra.calibrate();
...
void loop() {
    ...
    if (cond == 2 && (isRight()) && (millis() - entryTime) > 1500) { // first sharp turn
        cond = 3;
        sharpRight(250);
        entryTime = millis();
    }

    // right or left pad, it can be any of the randomly
    ...
    // second sharp turn, we apply custom logic here so that 5mayes turns perfectly to line with the hexagon
    if (cond == 4 && (isRight() || isFarRight()) && (millis() - entryTime) > 350) {
        ...
        rightWeight = 0;
        leftWeight = 0;
        // move forward until none of the sensors are on the line
        ...
        // move until before entering the hexagon
    ...
    if (cond == 7 && isLeft() && (millis() - entryTime) > 500) {
        ...
        // making the robot faster made the enter white flag function more often
        rightBaseSpeed = 200;
        leftBaseSpeed = 200;  
    }

    // enter white path, we invert the logic here
    ...
    // same logic as second sharp turn
    ...
    // zig zag, we just do hardcoded trash code here
...
// error caulcating function
// every sensor is assigned a weight, the more to the right it is the more negative
...
void PID() {
    ...
    int motorSpeed = Kp * error + Kd * diff; // PID formula
    // edit motor speeds depending on the error, add for right motor, subtract for left, as such the robot turns
...
// if most of the front sensors are white, then in front is white, used for some flags
...
// unused function, creates a delay for sensor values
void filterStep() {
    for (int i = 0; i < BUFFER_SIZE; i++) {
